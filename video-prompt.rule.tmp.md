...
```
[Camera motion + framing], [Subject (nano_name) + hành động TIẾN TRIỂN trong 8s], [Location ngắn], [Lighting/mood từ mood_map], [video_style], clean frame free of watermarks, logos, or AI icons, Audio: [ambient nhẹ]
```
...
- **Chống icon AI / watermark (KAIZEN 2026-07-09)**: MỌI video prompt VÀ image prompt ảnh phụ trong `video-prompts.md` phải kết bằng mệnh đề `clean frame free of watermarks, logos, or AI icons` (đặt trước `Audio:` ở video prompt; cuối prompt ở ảnh phụ) — chặn model tự vẽ icon AI/watermark/logo vào nội dung. Dùng "free of" (KHÔNG dùng no/not/without — Rule 4.6). Lưu ý: watermark Veo/Gemini do Google đóng lên video SAU render thì prompt không gỡ được (xử lý bằng tier trả phí/crop khi dựng). Image prompt clip .1 vẫn copy nguyên văn từ prompts.md, KHÔNG tự chèn mệnh đề (gate so khớp verbatim).
