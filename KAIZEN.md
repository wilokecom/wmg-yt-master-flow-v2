...
## [2026-07-09] Bước 4 âm thầm bỏ qua typewriter khi thiếu input/typewriter.json → metadata ra đời không có hiệu ứng đánh máy
- **Hiện tượng:** user báo `metadata.json` không có field `typewriter` nào. Gốc: khi thay SRT sang dự án mới (Dead Ponds), `input/typewriter.json` của dự án cũ bị xóa và KHÔNG được tạo lại cho dự án mới; Bước 4 chạy `export.py`, `load_typewriter()` trả `None`, nhánh `if tw:` bị bỏ qua HOÀN TOÀN im lặng — không log, không cảnh báo, không đề xuất. Video kể chuyện 65+ (có sẵn nhiều mốc mạnh: "$3,800", "2 năm sau", "~1,800 con cá chết", note của cha, "six figures", moral kết) ra đời mất trắng lớp text overlay mà không ai nhận ra tới tận review.
- **Gốc rễ:** typewriter được thiết kế "tùy chọn" nên khi vắng file nguồn, cả tầng máy (export.py) lẫn tầng người (CLAUDE.md Bước 4) đều KHÔNG có bước surface/đề xuất — "optional" bị hiểu thành "im lặng skip". Không có tín hiệu nào phân biệt "chủ ý không dùng typewriter" với "quên tạo file cho dự án mới".
- **Phòng ngừa:** (1) `export.py` — thêm nhánh `else` in **LƯU Ý** rõ ràng khi thiếu `input/typewriter.json` (không phải lỗi cứng vì typewriter vốn optional), gợi ý các mốc nên đặt cue + cách tạo file rồi chạy lại. (2) CLAUDE.md Bước 4 — quy định AI KHÔNG được im lặng đi tiếp: sau khi Bước 4 PASS mà thiếu file, AI phải chủ động ĐỀ XUẤT bộ cue (con số/time card/thesis/moral) cho user duyệt rồi tạo, TRỪ KHI user nói rõ không dùng. (3) Vá dữ liệu dự án hiện tại: tạo `input/typewriter.json` (9 cue thưa) → chạy lại export → metadata có `typewriter` + sinh `output/typewriter-cues.md`. Gate cứng cho việc "phải có typewriter": CỐ Ý KHÔNG thêm (typewriter thật sự optional, có video không cần) — dừng ở mức LƯU Ý + trách nhiệm AI đề xuất.
...
- **Phòng ngừa (đã codify vào CORE):** mệnh đề `clean frame free of watermarks, logos, or AI icons` trở thành ĐUÔI BẮT BUỘC của mọi style string: (1) Rule 2.1a — 2 preset style kết bằng mệnh đề, style anchor copy nguyên văn vào mọi image prompt nên mọi ảnh tự có (gate style-verbatim sẵn có tự cưỡng chế); (2) CLAUDE.md Bước 1 + config.example.json — preset `style` và `video_hook.video_style` mặc định đã gồm mệnh đề; (3) Rule 6.3 — video prompt nhận mệnh đề qua `[video_style]`, gate `--step3` cảnh báo video prompt thiếu; (4) Rule 1.4 + 2.3a — template ảnh ref nhân vật/bối cảnh kết bằng mệnh đề; (5) Rule 4.4 + `export.py` — mệnh đề (8 từ) không tính vào ngân sách 20-75 từ; (6) Rule 4.6 — ghi rõ ngoại lệ "free of" (vẫn cấm no/not/without). Project hiện tại: đã áp cho video-prompts.md (11 clip) + `video_style` trong config; 170 image prompt đã sinh trong prompts.md GIỮ NGUYÊN theo quyết định user (ảnh đã/đang vẽ) — icon vẫn xuất hiện ở ảnh tĩnh thì vá style anchor + chạy lại từ Bước 3. Lưu ý: watermark Veo/Gemini do Google ĐÓNG sau render không gỡ được bằng prompt (tier trả phí/crop khi dựng).
