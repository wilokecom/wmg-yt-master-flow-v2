...
|---|---|
| Ảnh thật điện ảnh | `photorealistic cinematic film still, 35mm film look, natural skin texture, soft depth of field, clean frame free of watermarks, logos, or AI icons` |
| Tranh vẽ điện ảnh | `painterly digital illustration, cinematic composition, rich brushwork texture, soft depth of field, clean frame free of watermarks, logos, or AI icons` |

- **Chống icon AI / watermark (KAIZEN 2026-07-09)**: style string PHẢI kết bằng mệnh đề `clean frame free of watermarks, logos, or AI icons` — vì style anchor được copy nguyên văn vào cuối MỌI prompt (Rule 4.1 Phần 5), đây là điểm hệ thống duy nhất đảm bảo mọi ảnh đều có chỉ dẫn khung hình sạch, chặn model tự vẽ icon AI/watermark/logo vào nội dung. Dùng "free of" (KHÔNG dùng no/not/without — Rule 4.6); mệnh đề này KHÔNG tính vào ngân sách từ Rule 4.4 (gate đã trừ). Tương tự, `video_hook.video_style` cũng kết bằng mệnh đề này (Rule 6.3). Lưu ý: watermark Veo/Gemini do Google đóng lên SAU render thì prompt không gỡ được.
...
```
Wide establishing shot of an empty [keywords của setting — nguyên văn cụm định danh + chi tiết], no people present rephrased as unoccupied, natural even lighting, [MEDIUM của style anchor], clean frame free of watermarks, logos, or AI icons
```
