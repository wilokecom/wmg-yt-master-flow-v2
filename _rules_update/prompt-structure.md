...
- **Cụm ĐẦU TIÊN (trước dấu phẩy đầu) của `settings[].keywords` là định danh bối cảnh** — prompt PHẢI chứa nguyên văn cụm này khi scene ở location đó. Bước 5 (`--unix-export`) nhận diện setting để gắn asset tag bằng đúng cụm này (khớp KHÔNG phân biệt hoa-thường); viết cụm rút gọn khác đi sẽ làm scene mất tag bối cảnh (xem KAIZEN 2026-07-06, 2026-07-07). Khi cần tiết kiệm từ, giữ cụm đầu nguyên vẹn và lược các cụm sau.
...
- Style chuẩn kết bằng mệnh đề chống icon AI/watermark `clean frame free of watermarks, logos, or AI icons` (Rule 2.1a, KAIZEN 2026-07-09) — copy nguyên văn là đủ, KHÔNG chèn thêm lần hai.
...
- Mệnh đề chống watermark `clean frame free of watermarks, logos, or AI icons` (8 từ, nằm cuối style anchor) KHÔNG tính vào ngân sách trên — gate `--step3` tự trừ trước khi đếm.
...
- **Positive framing** (theo guide chính thức của Google): mô tả cái MUỐN thấy, không mô tả cái không muốn. Viết `empty street` thay vì `no cars`; `bare desk` thay vì `without papers`. CẤM các từ phủ định trong prompt: no, not, without, never. (Ngoại lệ duy nhất: mệnh đề chống watermark trong style anchor dùng "free of" — cụm này nằm trong style anchor nên gate bỏ qua khi quét.)
