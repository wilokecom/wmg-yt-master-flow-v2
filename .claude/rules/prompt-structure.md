...
- **Cụm ĐẦU TIÊN (trước dấu phẩy đầu) của `settings[].keywords` là định danh bối cảnh** — prompt PHẢI chứa nguyên văn cụm này khi scene ở location đó. Bước 5 (`--unix-export`) nhận diện setting để gắn asset tag bằng đúng cụm này; viết cụm rút gọn khác đi sẽ làm scene mất tag bối cảnh (xem KAIZEN 2026-07-06). Khi cần tiết kiệm từ, giữ cụm đầu nguyên vẹn và lược các cụm sau.
