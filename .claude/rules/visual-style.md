...
- Khi Bước 2 phân loại mood cho scene, Bước 4 PHẢI dùng đúng visual keywords tương ứng từ bảng này.
- KHÔNG được tự sáng tạo visual keywords ngoài bảng.
- **mood_map CHỈ chứa ánh sáng + màu + khung hình. CẤM cụm SHOT SCALE** (`extreme close-up`, `wide shot`, `medium shot`, `low/high angle`, `macro`, `50mm`...) — mood được append vào MỌI prompt mang mood đó, nên một cụm scale lọt vào sẽ chọi với Phần 3 của những scene dùng shot khác (vd scene extreme wide vẫn bị kèm `extreme close-up`), khiến ảnh thật lệch khỏi quota Rule 4.2 dù bảng prompt vẫn "đủ quota". Gate `--step1` quét và báo LỖI cứng (KAIZEN 2026-07-20).
...
- Asset tag của setting = `id` viết hoa (Bước 6). Đặt tên ảnh ingredient bối cảnh đúng tag đó khi upload lên Uni-X.
- Format trong `character-refs.md`: mỗi setting cũng là heading `### TAG — Tên` + prompt trong code block (giống nhân vật, Rule 1.4) — để `--refs-list` parse được cả bối cảnh vào danh sách vẽ hàng loạt.
...
- **Bước 4**: prompt KHÔNG được chứa vật thể ngoài niên đại. Gate `--step4` quét `era.forbidden` — dính từ cấm là LỖI chặn.
