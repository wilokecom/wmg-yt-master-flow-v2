...
Các Rule trong `.claude/rules/` quy định NỘI DUNG đúng là gì (character nào, style nào, duration bao nhiêu...). Mục này quy định CÁCH LÀM VIỆC để tạo ra nội dung đó — áp dụng xuyên suốt Bước 1-5 và cả khi sửa lỗi ngoài pipeline.
...
- `--qc` báo "Lỗi: 0" KHÔNG đồng nghĩa Bước 5 xong nếu còn cảnh báo chưa xử lý — mỗi cảnh báo phải được sửa hoặc ghi rõ lý do chấp nhận, không im lặng bỏ qua vì đã thấy PASS.
- Mục "QC định tính" cuối `qc-report.md` phải thực sự được điền (tick từng ý + verdict cụ thể) trước khi coi Bước 5 hoàn tất — checklist để trống = QC chưa xong, dù tầng máy đã PASS.
...
└── output/
    ...
    ├── prompts-list.txt     ← (tùy chọn) prompt đánh số phẳng để copy chạy hàng loạt — `export.py --prompts-list`
    └── unix-batch-NN.txt    ← (tùy chọn) cú pháp Uni-X Studio, nhóm 100 ảnh/file — `export.py --unix-export`
```
...
3. Nói "chạy bước 2", "chạy bước 3", "chạy bước 4" lần lượt.
4. Hoặc nói "chạy toàn bộ pipeline" để chạy liên tục từ bước 1 → 4.
...
**Xuất định dạng phụ trợ cho công cụ vẽ ảnh (tùy chọn, chạy sau khi có `metadata.json`):**

- `python3 scripts/export.py --prompts-list` → `output/prompts-list.txt`: toàn bộ prompt đánh số `1. ...` `2. ...`, cách nhau 1 dòng trống, không kèm metadata — copy thẳng để chạy hàng loạt.
- `python3 scripts/export.py --unix-export` → `output/unix-batch-NN.txt`: cú pháp Uni-X Studio (mỗi ảnh gồm header `(Image Prompt i/n)`, dòng `Assets: TAG1, TAG2`, dòng `Prompt: ...` có gắn `[TAG]` tại đúng vị trí nhân vật/bối cảnh). Asset tag = `nano_name`/`id` viết hoa, khoảng trắng thành `_` (vd `Owen Consultant` → `OWEN_CONSULTANT`, setting `conference_room_7f` → `CONFERENCE_ROOM_7F`) — khi upload ảnh ingredient lên Uni-X, đặt tên đúng theo quy ước này. Tự chia nhóm tối đa 100 ảnh/file theo giới hạn 1 lượt vẽ của Uni-X.

Cả hai đều đọc từ `metadata.json` đã validate — không viết tay, chạy lại bất cứ lúc nào sau khi Bước 4 PASS.

---

### Bước 5: QC (BẮT BUỘC trước khi giao prompt sang Flow)
...
5. **Nguyên tắc gate:** AI KHÔNG tự tuyên bố "bước X đã xong". Chỉ có 4 lệnh sau mới quyết định:
   ...
   - Bước 5 xong ⟺ `python3 scripts/export.py --qc` → PASS + verdict định tính PASS trong qc-report.md
   Nếu phiên bị ngắt/tràn giữa chừng, phiên mới chỉ cần chạy 3 lệnh trên là biết chính xác đang dở ở đâu và thiếu gì.
